...
class DataFrameFormatter:
    ...
    def unite_honeypot_and_normal_traffic_dfs(self):
        ...
        combined_df = pd.concat(
            [self.suricata_df, self.normal_traffic_df], ignore_index=True, sort=False
        )
    ...
    def add_precalculations_all_dfs(self, calculate_ip_geoloc=False):
        ...
        self.suricata_df = self._apply_precalculations(
            self.suricata_df, calculate_ip_geoloc
        )

        # Apply to normal_traffic_df
        self.normal_traffic_df = self._apply_precalculations(
            self.normal_traffic_df, calculate_ip_geoloc
        )
