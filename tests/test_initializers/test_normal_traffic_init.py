...
class TestNormalTrafficDataFrameInitializer:
    ...
    def test_initialize_benign_traffic_correct_shape(self, temp_traffic_file, sample_normal_traffic_data):
        """Test DataFrame has correct number of rows when sample size >= data size"""
        initializer = NormalTrafficDataFrameInitializer(temp_traffic_file)
        df = initializer.initialize_benign_traffic(sample_size=10)
        assert len(df) == len(sample_normal_traffic_data)
    
    def test_initialize_benign_traffic_contains_expected_columns(self, temp_traffic_file):
        ...
        expected_columns = [
            ...
            'totalSourcePackets', 'totalDestinationPackets', 'source', 'destination',
            'sourcePort', 'destinationPort', 'protocolName', 'direction', 'Label'
        ]
